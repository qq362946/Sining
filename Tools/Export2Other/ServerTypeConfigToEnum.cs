...
{
    public static class ServerTypeConfigToEnum
    {
        private const string ConfigFile = "../../../../../Excel/ServerConfig.xlsx";
        private const string SaveConfigFile = "../../../../../Server/Hotfix/Base/Module/Server/ServerType.cs";
        ...
        public static void Run()
        {
            ...
            {
                if (table.TableName != "ServerTypeConfig") continue;

                for (var i = 4; i < table.Rows.Count; i++)
                {
                    var row = table.Rows[i];
                    
                    strBuilder.AppendLine($"\t\t{row[2]} = {row[1]}, \t// {row[3]}");
                }
