            ...
            while (Queue.TryDequeue(out var table))
            {
                ...
                for (var row = 4; row < currentTable.Rows.Count; row++)
                {
                    var config = (IConfig) Activator.CreateInstance(
                        _assembly.GetType($"Sining.Config.{currentTable.TableName}"));
            ...
            switch (type)
            {
                ...
                case "int[]":
                    propertyInfo.SetValue(config,
                        value.ToString().Split(",").Select(d => Convert.ToInt32(d)).ToArray());
                    break;
                case "long[]":
                    propertyInfo.SetValue(config,
                        value.ToString().Split(",").Select(d => Convert.ToInt64(d)).ToArray());
                    break;
                case "double[]":
                    propertyInfo.SetValue(config,
                        value.ToString().Split(",").Select(d => Convert.ToDouble(d)).ToArray());
                    break;
                case "string[]":
                    propertyInfo.SetValue(config, value.ToString().Split(",").ToArray());
                    break;
                case "float[]":
                    propertyInfo.SetValue(config,
                        value.ToString().Split(",").Select(d => Convert.ToSingle(d)).ToArray());
                    break;
                default:
